...
// TODO - Support the OUT parameter.
float
simplex(float3 v, /* out */ float3 gradient)
{
  ...
  float4 m =
    max(0.6 - float4(dot(x0, x0), dot(x1, x1), dot(x2, x2), dot(x3, x3)), 0.0);
  float4 m2 = m * m;
  float4 m4 = m2 * m2;
...
float
taylorInvSqrt(float r)
{
  return 1.79284291400159 - 0.85373472095314 * r;
}

float4
lessThan(float4 a, float4 b)
{
  return float4(
    //
    a.x < b.x ? 1 : 0,
...
simplex(float4 v)
{
  ...
  float4 j1 =
    permute(permute(permute(permute(i.w + float4(i1.w, i2.w, i3.w, 1.0)) + i.z +
