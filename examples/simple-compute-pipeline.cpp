...
void printSpan(
  const char* message,
  const std::span<float> container
) {
  std::cout << message;
...
int main() {
  ...
  printf("This creates functions, not sure why it's needed separate from the device.\n");
  ...
  auto bufferA = device.NewBuffer(bufferSize, mtlpp::ResourceOptions::StorageModeShared);
  auto bufferB = device.NewBuffer(bufferSize, mtlpp::ResourceOptions::StorageModeShared);
  auto bufferResult = device.NewBuffer(bufferSize, mtlpp::ResourceOptions::StorageModeShared);
  ...
  auto threadGroupSize = std::min(bufferSize, pipeline.GetMaxTotalThreadsPerThreadgroup());

  printf("This will divide up the threads with the data.\n");
  computeEncoder.DispatchThreadgroups(
    mtlpp::Size(threadGroupSize, 1, 1),
